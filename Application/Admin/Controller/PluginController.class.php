...
class PluginController extends BaseController
{
    ...
    public function uninstallLogin()
    {
        $condition['type'] = I('get.type');
        $condition['code'] = I('get.code');
        $update['status'] = I('get.install');
    ...
    public function install($condition, $update)
    {
        //$condition['type'] = I('get.type');
        //  $condition['code'] = I('get.code');
        // $update['status'] = I('get.install');
...
//                }
    ...
    private function scanPlugin()
    {
        ...
        $payment_list = M('SystemPlugin')->field('code')->select();
    ...
    {
        $d_list = M('SystemPlugin')->field('code,type')->select(); // 数据库
        ...
        foreach ($plugin_list as $pt => $pv) {
            ...
            foreach ($pv as $t => $v) {
                ...
                $new_arr[] = $tmp;
                // 对比数据库 本地有 数据库没有
                $is_exit = M('SystemPlugin')->where(array('type' => $pt, 'code' => $v['code']))->find();
                if (empty($is_exit)) {
                    $add['code'] = $v['code'];
                    $add['name'] = $v['name'];
     ...
     * 插件信息配置
     */
    public function setting($condition)
    {
        $model = M('SystemPlugin');
        $row = $model->where($condition)->find();
        if (!$row) {
    ...
    public function save($config, $data)
    {
        ...
        $row = $model->where('id = ' . $data['id'])->save(array('config_value' => $config));

        if ($row) {
            exit($this->success("操作成功", U('Admin/Plugin/loginList/action/page_list')));
        }
...
//        //2016-01-11 获取插件信息
//        $shipping_info = M('SystemPlugin')->where(array('code'=>$row['code'],'type'=>'shipping'))->find();
//        $result = M()->query($sql);
//
//        //获取配送名称
//        $this->assign('plugin',$row);
...
//        $desc = I('post.desc');
//        $code = I('post.code');
//        $row = M('SystemPlugin')->where(array('code'=>$code,'type'=>'shipping'))->save(array('desc'=>$desc));
//        if(!$row)
