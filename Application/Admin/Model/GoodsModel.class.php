...
class GoodsModel extends Model {
    ...
    {            
         ...
         if($_POST['item'])
         {
             ...
             $specItem = M('GoodsSpecItem')->getField('id,item');//规格项
                          
             $specGoodsPrice = M('GoodsSpecGoodsPrice'); // 实例化 商品规格 价格对象
         ...
         if($_POST['item_img'])
         {    
             M('GoodsSpecImage')->where("goods_id = $goods_id")->delete(); // 把原来是删除再重新插入
             foreach ($_POST['item_img'] as $key => $val)
             {                 
                 M('GoodsSpecImage')->data(array('goods_id'=>$goods_id ,'spec_image_id'=>$key,'src'=>$val))->add();
             }                                                    
         }
