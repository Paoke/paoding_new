...
class ArticleCatLogic extends RelationModel
{
     ...
     * @param   int     $flag      判断是否是文章选择分类时取出的信息
     * @return  mix
     */
    public function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0,$flag=0)
    {
        static $res = NULL;
            ...
            if ($data === false)
            {
                if(1==$flag) {
                    $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children FROM __PREFIX__article_cat AS c".
                        " LEFT JOIN __PREFIX__article_cat AS s ON s.parent_id=c.cat_id WHERE c.show_in_nav = 1".
