...
class GoodsOrderLogic extends RelationModel
{
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('GoodsOrder')->where($condition)->limit("$start,$page_size")->order($order)->select();
        return $res;
    }
    /*
     * 获取订单商品详情
    ...
    public function getOrderGoods($order_id){
        $sql = "SELECT g.*,o.*,(o.goods_num * o.member_goods_price) AS goods_total FROM __PREFIX__goods_order_goods o ".
    	...
    	if(!is_array($goods_id_arr)) return false;
    		foreach($goods_id_arr as $key => $val)
    		{
    			...
    			foreach($val as $k => $v)
    			{
    				...
    				{
    					$arr['spec_key'] = $k;
    					$spec_goods = M('GoodsSpecGoodsPrice')->where("goods_id = $key and `key` = '{$k}'")->find();
    ...
    public function orderActionLog($order_id,$action,$note=''){    	
        ...
        return M('GoodsOrderAction')->add($data);//订单操作记录
    }
    ...
    public function getGoodsAmount($order_id){
        $sql = "SELECT SUM(goods_num * goods_price) AS goods_amount FROM __PREFIX__goods_order_goods WHERE order_id = {$order_id}";
        $res = $this->query($sql);
    ...
    public function orderProcessHandle($order_id,$act){
    	...
    	switch ($act){
    		case 'pay': //付款
                        $order_sn = M('GoodsOrder')->where("order_id = $order_id")->getField("order_sn");
    	...
    	return M('GoodsOrder')->where("order_id=$order_id")->save($updata);//改变订单状态
    }
    ...
    public function deliveryHandle($data){
		...
		$did = M('GoodsDeliveryDoc')->add($data);
		$is_delivery = 0;
		foreach ($orderGoods as $k=>$v){
			...
			if($v['is_send'] == 0 && in_array($v['rec_id'],$selectgoods)){
				$res['is_send'] = 1;
				$res['delivery_id'] = $did;
				$r = M('GoodsOrderGoods')->where("rec_id=".$v['rec_id'])->save($res);//改变订单商品发货状态
				$is_delivery++;
			}
		}
		...
		M('GoodsOrder')->where("order_id=".$data['order_id'])->save($updata);//改变订单状态
    ...
    public function getAddressName($p=0,$c=0,$d=0){
        $p = M('SystemRegion')->where(array('id'=>$p))->field('name')->find();
        $c = M('SystemRegion')->where(array('id'=>$c))->field('name')->find();
        $d = M('SystemRegion')->where(array('id'=>$d))->field('name')->find();
    ...
    function delOrder($order_id){
    	$a = M('GoodsOrder')->where(array('order_id'=>$order_id))->delete();
    	$b = M('GoodsOrderGoods')->where(array('order_id'=>$order_id))->delete();
    	return $a && $b;
    }
