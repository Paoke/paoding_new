 ...
 * @package Home\Logic
 */
class RegLogic extends RelationModel
{
    private $r_model;
    private $r_model_arr = array('r' => 'register', 'rd' => 'register_department', 'rs' => 'register_spec', 'rc' => 'register_category');
    ...
    public function info($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
        ...
        if ($this->r_model == 'register') {
            $sql="SELECT ad.admin_id,ad.name,r.*,rd.id rd_id,rd.title rd_title,rs.id rs_id,rs.title rs_title FROM __PREFIX__register r LEFT JOIN __PREFIX__manage_admin ad ON ad.admin_id=r.create_user_id LEFT JOIN __PREFIX__register_department rd ON r.register_department_id=rd.id AND r.register_department_id=rd.id LEFT JOIN __PREFIX__register_spec rs ON r.register_category_id=rs.id WHERE r.is_deleted !=1 ORDER BY r.sort ASC ,r.id ASC";
