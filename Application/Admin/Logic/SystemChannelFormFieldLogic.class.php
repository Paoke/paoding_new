 ...
 * @package Admin\Logic
 */
class SystemChannelFormFieldLogic extends BaseLogic
{
    ...
    {
        parent::__construct();
        $this->DAO = D('SystemChannelFormField');
    }
     ...
     * 根据频道和表类型获取字段
     * **/
    public function getFields($channel, $type, $field='')
    {
        if(!$field){
            $field = 'name,title';
        }
        ...
        $where['type'] = $type;
        return $this->DAO->getFields($field, $where);
    }
