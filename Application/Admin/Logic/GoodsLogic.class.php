...
class GoodsLogic extends RelationModel
{
        ...
        foreach ($attributeList as $key => $val) {
            ...
            foreach ($curAttrVal as $k => $v) {
                ...
                if ($val['attr_input_type'] == 0) {
                    $str .= "<input type='text' class='form-control form-input' size='40' value='{$v['attr_value']}' name='attr_{$val['attr_id']}[]' />";
                }
                // 从下面的列表中选择（一行代表一个可选值）
                if ($val['attr_input_type'] == 1) {
                    $str .= "<select class='form-option' name='attr_{$val['attr_id']}[]'>";
            ...
            foreach ($v as $k2 => $v2) {
                ...
                if (array_key_exists($tmp_key, $old_goods_attr)) // 如果这个属性 原来就存在
                {
                    if ($old_goods_attr[$tmp_key]['attr_price'] != $attr_price) // 并且价格不一样 就做更新处理
                    {
                        ...
                        $GoodsAttr->where("goods_attr_id = $goods_attr_id")->save(array(
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        $spec = M('GoodsSpec')->getField('id,name'); // 规格表
        $specItem = M('GoodsSpecItem')->getField('id,item,spec_id'); // 规格项
        $keySpecGoodsPrice = M('GoodsSpecGoodsPrice')->where('goods_id = ' . $goods_id)->getField('key,key_name,price,store_count,bar_code,sku'); // 规格项
    ...
    {
        $list = M('GoodsSpec')->where("type_id = $type_id")
            ->order('`order` desc')
            ->select();
        // $list = M('GoodsSpec')->where("1=1")->order('`order` desc')->select();
        $str = '';
    ...
    {
        $list = M('GoodsAttribute')->where("type_id = $type_id and attr_index > 0 ")
            ->order('`order` desc')
            ->select();
        // $list = M('GoodsSpec')->where("1=1")->order('`order` desc')->select();
        $str = '';
    ...
    {
        $brandList = M('GoodsBrand')->select();
        $brandIdArr = M('GoodsBrand')->where("name in (select `name` from `" . C('DB_PREFIX') . "goods_brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');
