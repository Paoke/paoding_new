 ...
 * @package Home\Logic
 */
class ManageUsersLogic extends RelationModel
{
    /*
     * 登陆
     */
    public function login($username, $password)
    {
        ...
        if (strlen($username) > 11)
            $result = array('status' => -4, 'msg' => '请输入正确的手机号码');
        if (strlen($password) < 6 || strlen($password) > 15)
            $result = array('status' => -5, 'msg' => '请输入6-15位密码，仅限数字与字母');
        $user = M('ManageUsers')->where("mobile='{$username}'")->find();
        if (!$user) {
    ...
    public function thirdLogin($data = array())
    {
        ...
        if (!$user) {
            $wx_auto_register = M('config')->field("value")->where('name="wx_auto_register"')->find();
            if ($wx_auto_register) {
                //账户不存在 注册一个
                $map['password'] = '';
                $map['openid'] = $openid;
                ...
                $map['sex'] = $data['sex'];
                $map['mobile'] = 'wechat_' . get_rand_str(12, 0, 0);
                $map['first_leader'] = cookie('first_leader'); // 推荐人id
                if ($_GET['first_leader'])
                    $map['first_leader'] = $_GET['first_leader']; // 微信授权登录返回时 get 带着参数的
                // 如果找到他老爸还要找他爷爷他祖父等
                if ($map['first_leader']) {
                    $first_leader = M('ManageUsers')->where("user_id = {$map['first_leader']}")->find();
                ...
                $row = M('ManageUsers')->add($map);
                $user = get_user_info($openid, 3, $oauth);
                // 会员注册送优惠券
                $coupon = M('GoodsCoupon')->where("send_end_time > " . time() . " and ((createnum - send_num) > 0 or createnum = 0) and type = 2")->select();
                foreach ($coupon as $key => $val) {
                    // 送券
                    M('GoodsCouponList')->add(array('cid' => $val['id'], 'type' => $val['type'], 'uid' => $user[user_id], 'send_time' => time()));
                    M('GoodsCoupon')->where("id = {$val['id']}")->setInc('send_num'); // 优惠券领取数量加一
                }
            } else {
                return array('status' => 2, 'msg' => '未开启自动登录', 'result' => $user);
            }
        }
     ...
     * @return array
     */
    public function bindPhone($userid, $username, $password, $password2)
    {
        $is_validated = 0;
        ...
        if ($is_validated != 1)
            return array('status' => -1, 'msg' => '请用手机号绑定', 'result' => '');
        if (!$username || !$password)
            return array('status' => -1, 'msg' => '请输入手机号或密码', 'result' => '');
        //验证两次密码是否匹配
        if ($password2 != $password)
            return array('status' => -1, 'msg' => '两次输入密码不一致', 'result' => '');
        //验证是否存在用户名
        if (get_user_info($username, 2))
            return array('status' => -1, 'msg' => '此号码已被绑定', 'result' => '');
        $map['user_id'] = $userid;
        $map['mobile'] = $username;
        $map['password'] = encrypt($password);
        $row = M('ManageUsers')->save($map);

        if (!$row)
            return array('status' => -1, 'msg' => '绑定失败', 'result' => '');
        $map2['user_id'] = $userid;
        $user = M('ManageUsers')->where($map2)->find();
        return array('status' => 1, 'msg' => '绑定成功', 'result' => $user);
    }
    ...
    public function reg($username, $password, $password2)
    {
        ...
        $map['password'] = encrypt($password);
        $map['reg_time'] = time();
        $map['head_pic'] = '__PUBLIC__/images/m_pic.png';
        $map['sex'] = 1;
        $map['first_leader'] = cookie('first_leader'); // 推荐人id
        // 如果找到他老爸还要找他爷爷他祖父等
        if ($map['first_leader']) {
        ...
        foreach ($coupon as $key => $val) {
            // 送券
            M('GoodsCouponList')->add(array('cid' => $val['id'], 'type' => $val['type'], 'uid' => $user_id, 'send_time' => time()));
            M('GoodsCoupon')->where("id = {$val['id']}")->setInc('send_num'); // 优惠券领取数量加一
        }
    ...
    public function get_info($user_id)
    {
        ...
        $user_info = M('ManageUsers')->where("user_id = {$user_id}")->find();
        if (!$user_info)
            return false;

        $user_info['coupon_count'] = M('GoodsCouponList')->where("uid = $user_id and use_time = 0")->count(); //获取优惠券列表
        $user_info['collect_count'] = M('goods_collect')->where(array('user_id' => $user_id))->count(); //获取收藏数量

        $user_info['waitPay'] = M('GoodsOrder')->where("user_id = $user_id " . C('WAITPAY'))->count(); //待付款数量
        $user_info['waitSend'] = M('GoodsOrder')->where("user_id = $user_id " . C('WAITSEND'))->count(); //待发货数量
        $user_info['waitReceive'] = M('GoodsOrder')->where("user_id = $user_id " . C('WAITRECEIVE'))->count(); //待收货数量
    ...
    {
        $last_order = M('GoodsOrder')->where("user_id = {$user_id}")->order('order_id DESC')->find();
        return $last_order;
    }
    ...
    {
        $sql = "SELECT og.*,g.original_img FROM __PREFIX__goods_order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE order_id = " . $order_id;
...
//        $type = I('get.type',0);
        ...
        $count = M('FundAccountLog')->where($where ? $where : 'user_id = ' . $user_id)->count();
        $Page = new Page($count, 16);
        $logs = M('FundAccountLog')->where($where ? $where : 'user_id = ' . $user_id)->order('change_time desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function get_coupon($user_id, $type = 0)
    {
        ...
        //获取数量
        $sql = "SELECT count(l.id) as total_num FROM __PREFIX__goods_coupon_list" .
            " l LEFT JOIN __PREFIX__goods_coupon" .
        ...
        $sql = "SELECT l.*,c.name,c.money,c.use_end_time,c.condition FROM __PREFIX__goods_coupon_list" .
            " l LEFT JOIN __PREFIX__goods_coupon" .
    ...
    public function get_comment($user_id, $status = 2)
    {
        if ($status == 1) {
            //已评论
            $count2 = M('')->query("select count(*) as count from `__PREFIX__comment`  as c inner join __PREFIX__goods_order_goods as g on c.goods_id = g.goods_id and c.order_id = g.order_id where c.user_id = $user_id");
            $count2 = $count2[0]['count'];

            $page = new Page($count2, 10);
            $sql = "select c.*,g.*,(select order_sn from  __PREFIX__goods_order where order_id = c.order_id ) as order_sn  from  __PREFIX__goods_comment as c inner join __PREFIX__goods_order_goods as g on c.goods_id = g.goods_id and c.order_id = g.order_id where c.user_id = $user_id order by c.add_time desc LIMIT {$page->firstRow},{$page->listRows}";
        } else {
            $countsql = " select count(1) as comment_count from __PREFIX__goods_order_goods as og
        	left join __PREFIX__goods_order as o on o.order_id = og.order_id where o.user_id = $user_id  and og.is_send = 1 ";
            $where = '';
            if ($status == 0) {
            ...
            $sql = " select *  from __PREFIX__goods_order_goods as og
            left join __PREFIX__goods_order as o on o.order_id = og.order_id  where o.user_id = $user_id and og.is_send = 1
    ...
    public function add_comment($add)
    {
        ...
        //检查订单是否已完成
        $order = M('GoodsOrder')->where("order_id = {$add[order_id]} AND user_id = {$add[user_id]}")->find();
        if ($order['order_status'] != 2)
            return array('status' => -1, 'msg' => '该笔订单还未完成', 'result' => '');

        //检查是否已评论过
        $goods = M('GoodsComment')->where("order_id = {$add[order_id]} AND goods_id = {$add[goods_id]}")->find();
        if ($goods)
            return array('status' => -1, 'msg' => '您已经评论过该商品', 'result' => '');

        $row = M('GoodsComment')->add($add);
        if ($row) {
            //更新订单商品表状态
            M('GoodsOrderGoods')->where(array('goods_id' => $add[goods_id], 'order_id' => $add[order_id]))->save(array('is_comment' => 1));
            M('goods')->where(array('goods_id' => $add[goods_id]))->setInc('comment_count', 1); // 评论数加一
            // 查看这个订单是否全部已经评论,如果全部评论了 修改整个订单评论状态
            $comment_count = M('GoodsOrderGoods')->where("order_id ='{$add[order_id]}' and is_comment = 0")->count();
            if ($comment_count == 0) // 如果所有的商品都已经评价了 订单状态改成已评价
            {
                M('GoodsOrder')->where("order_id ='{$add[order_id]}'")->save(array('order_status' => 4));
            }
    ...
    public function update_email_mobile($email_mobile, $user_id, $type = 1)
    {
        ...
        $is_exist = M('ManageUsers')->where($condition)->find();
        if ($is_exist)
            return false;
        unset($condition[$field]);
        $condition['user_id'] = $user_id;
        $validate = $field . '_validated';
        M('ManageUsers')->where($condition)->save(array($field => $email_mobile, $validate => 1));
        return true;
    }
    ...
    {
        $model = M('ManageUsers')->where("user_id = " . $user_id);
        $row = $model->save($post);
        if ($row === false)
            return false;
        return true;
    ...
    public function add_address($user_id, $address_id = 0, $data)
    {
        ...
        if ($address_id > 0) {
            ...
            $address = M('ManageUserAddress')->where(array('address_id' => $address_id, 'user_id' => $user_id))->find();
            if ($post['is_default'] == 1 && $address['is_default'] != 1)
                M('ManageUserAddress')->where(array('user_id' => $user_id))->save(array('is_default' => 0));
            $row = M('ManageUserAddress')->where(array('address_id' => $address_id, 'user_id' => $user_id))->save($post);
            if (!$row)
        ...
        $c = M('ManageUserAddress')->where("user_id = {$post['user_id']}")->count();
        if ($c == 0) $post['is_default'] = 1;

        $address_id = M('ManageUserAddress')->add($post);
        //如果设为默认地址
        ...
        if ($post['is_default'] == 1)
            M('ManageUserAddress')->where($map)->save(array('is_default' => 0));
        if (!$address_id)
    ...
    {
        M('ManageUserAddress')->where(array('user_id' => $user_id))->save(array('is_default' => 0)); //改变以前的默认地址地址状态
        $row = M('ManageUserAddress')->where(array('user_id' => $user_id, 'address_id' => $address_id))->save(array('is_default' => 1));
        if (!$row)
            return false;
        return true;
    }
    ...
    public function password($user_id, $old_password, $new_password, $confirm_password, $is_update = true)
    {
        ...
        $row = M('ManageUsers')->where("user_id='{$user_id}'")->save(array('password' => encrypt($new_password)));
        if (!$row)
    ...
    /**
     * 手机端密码找回 复写
     * @param $user_id 修改的用户id
     * @param $new_password 新密码
     * @param $confirm_password 确认密码
     * @return array 返回数组
     */
    public function password2($user_id, $new_password, $confirm_password)
    {
        $data = $this->get_info($user_id);
        $user = $data['result'];
        if (strlen($new_password) < 6)
            return array('status' => -1, 'msg' => '输入有误，请输入6-15位的字母与数字', 'result' => '');
        if ($new_password != $confirm_password)
            return array('status' => -1, 'msg' => '两次密码输入不一致', 'result' => '');
        if (encrypt($new_password) == $data['result']['password'])
            return array('status' => -1, 'msg' => '修改的密码不能与原密码相同', 'result' => '');
        $users = M('ManageUsers');
        $data['password'] = encrypt($new_password);
        $row = $users->where("user_id=" . $user['user_id'])->save($data);
        if (!$row)
            return array('status' => -1, 'msg' => '修改失败', 'result' => '');
        return array('status' => 1, 'msg' => '修改成功', 'result' => '');
    }
    ...
    {
        $order = M('GoodsOrder')->where(array('order_id' => $order_id, 'user_id' => $user_id))->find();
        ...
        //获取记录表信息
        //$log = M('FundAccountLog')->where(array('order_id'=>$order_id))->find();
        //有余额支付的情况
        ...
        $row = M('GoodsOrder')->where(array('order_id' => $order_id, 'user_id' => $user_id))->save(array('order_status' => 3));
        ...
        M('GoodsOrderAction')->add($data);//订单操作记录
