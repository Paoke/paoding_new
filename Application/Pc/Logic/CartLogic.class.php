...
class CartLogic extends RelationModel
{
    ...
    {       
        ...
        $specGoodsPriceList = M('GoodsSpecGoodsPrice')->where("goods_id = $goods_id")->getField("key,key_name,price,store_count,sku"); // 获取商品对应的规格价钱 库存 条码
        ...
        {
            $flash_sale = M('GoodsFlashSale')->where("id = {$goods['prom_id']} and ".time()." > start_time and ".time()." < end_time and goods_num > buy_num")->find(); // 限时抢购活动
            if($flash_sale){
...
function cart_freight2($shipping_code,$province,$city,$district,$weight)
{
   ...
   return $freight;  
    ...
    public function getCouponMoney($user_id, $coupon_id,$mode)
    {
        ...
        $coupon = M('GoodsCoupon')->where("id = {$couponlist['cid']}")->find(); // 获取 优惠券类型表
    ...
    public function getCouponMoneyByCode($couponCode,$order_momey)
    {
        ...
        $coupon = M('GoodsCoupon')->where("id = {$couponlist['cid']}")->find(); // 获取优惠券类型表
    ...
    public function addOrder($user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price)
    {
        ...
        $shipping = M('SystemPlugin')->where("code = '$shipping_code'")->find();
        $data = array(
        ...
        if($coupon_id > 0){
        	...
        	M('CouponList')->where("id = $coupon_id")->save($data3);
                ...
                M('GoodsCoupon')->where("id = $cid")->setInc('use_num'); // 优惠券的使用数量加一
        }
        // 3 扣除积分 扣除余额
        if($car_price['pointsFee']>0)
        	M('ManageUser')->where("user_id = $user_id")->setDec('pay_points',($car_price['pointsFee'] * tpCache('shopping.point_rate'))); // 消费积分 
        if($car_price['balance']>0)
        	M('ManageUser')->where("user_id = $user_id")->setDec('user_money',$car_price['balance']); // 抵扣余额         
        // 4 删除已提交订单商品
        ...
        // 如果使用了积分或者余额才记录
        ($data4['user_money'] || $data4['pay_points']) && M("FundAccountLog")->add($data4);
   ...
   {
       $group_buy = M('GoodsGroupBuy')->where("goods_id = $goods_id and ".time()." >= start_time and ".time()." <= end_time ")->find(); // 找出这个商品                      
       if(empty($group_buy))       
            return 0;
