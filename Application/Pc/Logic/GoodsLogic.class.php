...
class GoodsLogic extends RelationModel
{
   ...
   {       
        ...
        $list_brand = M('GoodsBrand')->where("id in(select brand_id from ".C('DB_PREFIX')."goods where brand_id > 0 and goods_id in($goods_id_str))")->limit('30')->select();  //where("goods_id in($goods_id_str)")->select();
   ...
   {       
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';       
        $spec_key = M('GoodsSpecGoodsPrice')->query("select group_concat(`key` separator  '_') as `key` from __PREFIX__goods_spec_goods_price where goods_id in($goods_id_str)");  //where("goods_id in($goods_id_str)")->select();
        ...
        $spec = M('GoodsSpec')->getField('id,name');
        $spec_item = M('GoodsSpecItem')->getField('id,spec_id,item');                
   ...
   public function get_spec_item_goods_id($spec_item_id,$mode = 0)
   {
       ...
       {           
         ...
         $sql = "select * from (
                  select *,concat('_',`key`,'_') as key2 from __PREFIX__goods_spec_goods_price as a
              ) b  $where";
   ...
   public function get_attr_goods_id($attr_id, $mode = 0)
   {
       // select * from `tp_goods_attr` where attr_id = 185 and (attr_value in('白色','黑色'))        
        ...
        {
                $sql = "select goods_id from __PREFIX__goods_goods_attr  where attr_id = $key and attr_value in ('".implode("','",$val)."')";
                $result = $Model->query($sql);
   ...
   {      
        $c1 = M('GoodsComment')->where("is_show = 1 and  goods_id = $goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(4,5)")->count(); // 好评
        $c2 = M('GoodsComment')->where("is_show = 1 and  goods_id = $goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(3)")->count(); // 中评
        $c3 = M('GoodsComment')->where("is_show = 1 and  goods_id = $goods_id and parent_id = 0 and  ceil((deliver_rank + goods_rank + service_rank) / 3) in(1,2)")->count(); // 差评
   ...
   public function get_spec($goods_id){
	   	...
	   	$keys = M('GoodsSpecGoodsPrice')->where("goods_id = $goods_id")->getField("GROUP_CONCAT(`key` SEPARATOR '_') ");
	   	$filter_spec = array();
	   	if($keys)
	   	{
	   		$specImage =  M('GoodsSpecImage')->where("goods_id = $goods_id and src != '' ")->getField("spec_image_id,src");// 规格对应的 图片表， 例如颜色
	   		$keys = str_replace('_',',',$keys);
	   		$sql  = "SELECT a.name,a.order,b.* FROM __PREFIX__goods_spec AS a INNER JOIN __PREFIX__goods_spec_item AS b ON a.id = b.spec_id WHERE b.id IN($keys) ORDER BY a.order";
...
function get_filter_menu($filter_param,$action)
{
    ...
    if(!empty($filter_param['brand_id']))
    {
        $brand_list = M('GoodsBrand')->getField('id,name');
    ...
    {
       $spec = M('GoodsSpec')->getField('id,name');
       $spec_item = M('GoodsSpecItem')->getField('id,item');
...
function getGoodsIdBySpec($spec)
{
    ...
    foreach ($spec_group as $k => $v)
    {
        ...
        //    $arr = M('ActivitySpecGoodsPrice')->where($where)->getField('goods_id',true);
         $sql = "select * from (
                  select *,concat('_',`key`,'_') as key2 from __PREFIX__goods_spec_goods_price as a
              ) b  $where";
        $Model  = new \Think\Model();  
