...
function get_user_info($user_id_or_name, $type = 0, $oauth = '')
{
    ...
    $user = M('ManageUsers')->where($map)->find();
    return $user;
}
...
{
    $level_info = D('ManageUserLevel')->order('level_id')->select();
    $total_amount = D('GoodsOrder')->where("user_id=$user_id AND order_status=2 or order_status=4")->sum('order_amount');
    ...
    return M('ManageUsers')->where("user_id=$user_id")->save($updata);
}
...
{
    $orderGoodsArr = M('GoodsOrderGoods')->where("order_id = $order_id")->select();
    foreach ($orderGoodsArr as $key => $val) {
        // 有选择规格的商品
        if (!empty($val['spec_key'])) {   // 先到规格表里面扣除数量 再重新刷新一个 这件商品的总数量
            M('GoodsSpecGoodsPrice')->where("goods_id = {$val['goods_id']} and `key` = '{$val['spec_key']}'")->setDec('store_count', $val['goods_num']);
...
function accountLog($user_id, $user_money = 0, $pay_points = 0, $desc = '', $distribut_money = 0)
{
    ...
    /* 更新用户信息 */
    $sql = "UPDATE __PREFIX__manage_users SET user_money = user_money + $user_money," .
        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    if (D('ManageUsers')->execute($sql)) {
        M('FundAccountLog')->add($account_log);
        return true;
    } else {
        return false;
    }
...
function logOrder($order_id, $action_note, $status_desc, $user_id = 0)
{
    ...
    return M('GoodsOrderAction')->add($action_info);
}
...
function get_region_list()
{
    //获取地址列表 缓存读取
    if (!S('region_list')) {
        $region_list = M('SystemRegion')->select();
...
{
    $lists = M('ManageUserAddress')->where(array('user_id' => $user_id))->select();
    return $lists;
}
...
{
    $data = M('ManageUserAddress')->where(array('user_id' => $user_id, 'address_id' => $address_id))->find();
    return $data;
}
...
{
    $data = M('ManageUserAddress')->where(array('user_id' => $user_id, 'is_default' => 1))->find();
    return $data;
}
...
{
    // 如果这笔订单已经处理过了
    $count = M('GoodsOrder')->where("order_sn = '$order_sn' and pay_status = 0")->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
    if ($count == 0) return false;
    // 找出对应的订单
    $order = M('GoodsOrder')->where("order_sn = '$order_sn'")->find();
    // 修改支付状态  已支付
    M('GoodsOrder')->where("order_sn = '$order_sn'")->save(array('pay_status' => 1, 'pay_time' => time()));
    // 减少对应商品的库存
    ...
    if ($distribut_condition == 1)  // 购买商品付款才可以成为分销商
        M('ManageUsers')->where("user_id = {$order['user_id']}")->save(array('is_distribut' => 1));
}
...
{
    $order = M('GoodsOrder')->where(array('order_id' => $id))->find();
    ...
    $row = M('GoodsOrder')->where(array('order_id' => $id))->save($data);
    if (!$row)
...
{
    $order_goods = M('GoodsOrderGoods')->where("order_id=" . $order['order_id'])->cache(true)->select();
    //查找购买商品送优惠券活动
    foreach ($order_goods as $val) {
        if ($val['prom_type'] == 3) {
            $prom = M('GoodsPromGoods')->where('type=3 and id=' . $val['prom_id'])->find();
            if ($prom) {
                $coupon = M('GoodsCoupon')->where("id=" . $prom['expression'])->find();//查找优惠券模板
                if ($coupon && $coupon['createnum'] > 0) {
                    $remain = $coupon['createnum'] - $coupon['send_num'];//剩余派发量
                    if ($remain > 0) {
                        $data = array('cid' => $coupon['id'], 'type' => $coupon['type'], 'uid' => $order['user_id'], 'send_time' => time());
                        M('GoodsCouponList')->add($data);
                        M('GoodsCoupon')->where("id = {$coupon['id']}")->setInc('send_num'); // 优惠券领取数量加一
                    }
                }
            }
        }
    ...
    $prom = M('GoodsPromGoods')->where("type>1 and end_time>$pay_time and start_time<$pay_time and money<=" . $order['order_amount'])->order('money desc')->find();
    if ($prom) {
        if ($prom['type'] == 3) {
            $coupon = M('GoodsCoupon')->where("id=" . $prom['expression'])->find();//查找优惠券模板
            if ($coupon) {
    ...
    $points = M('GoodsOrderGoods')->where("order_id = {$order[order_id]}")->sum("give_integral * goods_num");
...
function get_goods_promotion($goods_id, $user_id = 0)
{
    ...
    if ($goods['prom_type'] == 1) {//抢购
        $prominfo = M('GoodsFlashSale')->where($where)->cache(true)->find();
        if (!empty($prominfo)) {
            if ($prominfo['goods_num'] == $prominfo['buy_num']) {
                ...
                $order_id_arr = M('GoodsOrder')->where($where)->getField('order_id', true);
                if ($order_id_arr) {
                    $goods_num = M('GoodsOrderGoods')->where("prom_id=" . $goods['prom_id'] . " and order_id in (" . implode(',', $order_id_arr) . ")")->sum('goods_num');
                    if ($goods_num < $prominfo['']) {
    ...
    if ($goods['prom_type'] == 2) {//团购
        $prominfo = M('GoodsGroupBuy')->where($where)->cache(true)->find();
        if (!empty($prominfo)) {
            if ($prominfo['goods_num'] == $prominfo['buy_num']) {
                $prom['is_end'] = 2;//已售馨
            } else {
    ...
    if ($goods['prom_type'] == 3) {//优惠促销
        ...
        $prominfo = M('GoodsPromGoods')->where($where)->cache(true)->find();
        if (!empty($prominfo)) {
            if ($prominfo['type'] == 0) {
...
function get_order_promotion($order_amount)
{
    ...
    $now = time();
    $prom = M('GoodsPromGoods')->where("type<2 and end_time>$now and start_time<$now and money<=$order_amount")->order('money desc')->find();
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    $user = M('ManageUsers')->where("user_id = $user_id")->find();// 找出这个用户
 ...
 * 查询模块ID
 */
function get_mod_id($ctl, $act)
{
    $map['ctl'] = $ctl;
    $map['act'] = $act;
    $mod_id = M('system_module')->where($map)->getField('mod_id');
...
/**
 * @param $returnArr array 数组
 * 将传入的数组解析成API，另外将msg格式转换成中文。
 */
function json_return($returnArr = array())
{
    header("Content-type:text/json;charset=utf-8");
    if ($returnArr) {
        $returnArr["msg"] = urlencode($returnArr["msg"]);
        exit(urldecode(json_encode($returnArr)));
    } else {
        return;
    }
}
...
/**
 * @param $array  array 截取的数据
 * @param $name  string 截取的字段
 * @param $rename  string 重新命名截取的字段
 * @param $length  int 截取的长度
 * @return mixed 字符串截取后的数据
 */
function string_str($array, $name, $rename, $length)
{
    foreach ($array as $key => $value) {
        $array[$key][$rename] = $value[$name];
        if (mb_strlen($value[$name]) > $length) {
            $array[$key][$name] = mb_substr($value[$name], 0, $length, 'utf-8') . '...';
        } else {
 ...
 */
function getChannelTable($channel = "", $type = 0)
{

    $prefix = session('site_name') . '_';
    $info = M()->table($prefix . "system_channel_table_config")->field("id,table_format,table_name")->where("channel='$channel' AND type=$type")->find();
    $info['table_join'] = '__' . strtoupper(str_replace($prefix, '', $info['table_name'])) . '__';
    return $info;
}
 ...
 * 获取频道表
 * **/
function getTableStr($channel, $type, $key){
    return getChannelTable($channel,$type)[$key];
}
