...
<div class="center">
	...
	<nav class="navigation">
		<ul class="navigation-list ul-reset">
			...
			<li class="navigation-item ib">
				<a href="/index.php/Pc/Article/data_list/channel/zx" class='navigation-link selected-link'>
					资讯
				</a>
			</li>
		</ul>
		...
		<div class="center">
			<div class="article_tab">
				<ul class="science_tabUl" style="width: 100%;">
					<li class="science_tabLi"><a class='science_tabLi_a science_tabLi_a_active' href="/index.php/Pc/Article/data_list/channel/<?php echo ($channel); ?>/type/1">全部</a>
					</li>

					<li class="science_tabLi" v-for="category in categoryData">
						<!--<a class='science_tabLi_a <?php if($_GET['id'] == $v['category_id']): ?>science_tabLi_a_active<?php endif; ?>' href="<?php echo U('Article/index',array('type_id'=>$v['art_cat_id']));?>"><?php echo ($v['cat_name']); ?></a>-->
						<a class="science_tabLi_a" v-text="category.cat_name" v-bind:id="category.id" @click="search(category.id)"></a>
					</li>
			...
			<ul class="article_listUl" >
				<!--<?php if(($i-1)%4=='0'): ?>-->
				<!--<li class="article_listLi" style="margin-left:0;">-->
				<!--<?php else: endif; ?>-->
				<li class="article_listLi" v-for="list in listData"  @click="showDetail(list.link)">
...
<script>
	...
	function getUser() {
		...
		if(ifWeChatLogin == 1) {
			...
			$.get(url, function (res) {
				window.location.href="/index.php/Pc/Article/data_list/channel/zx"
			})
		}
	}
	function  WeChatLogin() {
		...
		$.get(url, function (res) {
			if(res.result == 1) {
				window.location.href="https://open.weixin.qq.com/connect/qrconnect?appid="+res.data['appid']+"&redirect_uri=http://"+res.data['area_name']+"//index.php/Pc/Article/data_list/channel/zx/ifWeChatLogin/1&response_type=code&scope=snsapi_login&state=2015#wechat_redirect";
			} else {
				layer.msg("系统繁忙请稍后再试！");
			}
		})
	...
	var vm = new Vue({
		...
		methods:{
			...
			search:function(id){
				var _this = this;
				var url = "/<?php echo (API_PATH); ?>/ChannelIndex/index/action/dataList/channel/<?php echo ($channel); ?>/type/1/category_id/"+id;
				var data={
					"order_field":"create_time",
					"order_by":"DESC"
				};
