	...
	<div class="center user_center">
		<div class="user_widget">
			...
			<div class="user_widget_body">
				...
				<a href="/index.php/Pc/User/user_info" class='desc_list active'>基础资料</a>
				...
				<form>
					...
					<div class="user_desc" >
						...
						<template v-if="userInfo.mobile==''">
							...
							<a href="<?php echo U('User/user_security');?>" style="font-size:14px;">立即绑定</a>
						</template>
						<template v-else>
							<input class="user_input" v-model="userInfo.mobile"  disabled="disabled" style="background:#CCCCCC"  type="text"/>
							<a href="<?php echo U('User/user_security');?>"  style="font-size:14px;">修改绑定号码</a>
						</template>
					</div>
...
<script>
	...
	function getUser() {
		...
		if(ifWeChatLogin == 1) {
			...
			$.get(url, function (res) {
				window.location.href="/index.php/Pc/User/user_info"
			})
		}
	}
	function  WeChatLogin() {
		...
		$.get(url, function (res) {
			if(res.result == 1) {
				window.location.href="https://open.weixin.qq.com/connect/qrconnect?appid="+res.data['appid']+"&redirect_uri=http://"+res.data['area_name']+"//index.php/Pc/User/user_info/ifWeChatLogin/1&response_type=code&scope=snsapi_login&state=2015#wechat_redirect";
			} else {
				layer.msg("系统繁忙请稍后再试！");
			}
		})
	...
	function save(){
		var url = "/<?php echo (API_PATH); ?>/User/user_info/action/edit/id/<?php echo ($userId); ?>";
...
<script>
	...
	function readFile(){
		...
		reader.onload = function(e){
			aaa.src=this.result;
			$.ajax({
				url:"/<?php echo (API_PATH); ?>/User/user_info/action/update_head/id/<?php echo ($userId); ?>",
