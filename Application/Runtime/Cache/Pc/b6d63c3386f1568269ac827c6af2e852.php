...
<head>
	...
	<link rel="stylesheet" href="/Template/5u/pc/Static/css/css_banner.css">
	<link rel="stylesheet" type="text/css" href="/Template/5u/pc/Static/css/style-activity.css"/>
	<style>
...
<section>
	<div class="demo">
		...
		<div class="slider"><!--主体结构，请用此类名调用插件，此类名可自定义-->
			<ul>
				<?php if(is_array($top)): foreach($top as $key=>$vo): ?><li><a href="/index.php/Pc/Activity/detail/channel/<?php echo ($channel); ?>/type/1/data_id/<?php echo ($vo["id"]); ?>"><img src="<?php echo ($vo["cover_url"]); ?>" /></a></li><?php endforeach; endif; ?>
			</ul>
		</div>
	</div>
</section>
<script src="/Template/5u/pc/Static/js/jquery.min.js"></script>
<script src="/Template/5u/pc/Static/js/YuxiSlider.jQuery.min.js"></script>
<script>
	$(".slider").YuxiSlider({
		width:800, //容器宽度
		height:450, //容器高度
			...
			<ul class="activity_listUl" >
				<li class="activity_listLi" v-for="list in listData"  @click="search(list.id)">
					...
					<p class="activity_listLi_content" >
						<img src="/Template/5u/pc/Static/img/activity_list1.png"/>{{list.formal_start_time}}-{{list.formal_end_time}}
					</p>
					<p class="activity_listLi_content" >
						<img src="/Template/5u/pc/Static/img/activity_list2.png"/>{{list.address}}
					</p>
					<p class="activity_listLi_content">
						<img src="/Template/5u/pc/Static/img/activity_list3.png"/>限额{{list.renshu}}人
					</p>
					<p class="activity_listLi_content">
						<img src="/Template/5u/pc/Static/img/activity_list4.png"/>{{list.cost}}
					</p>
					</a>
				</li>
			</ul>
	...
	<div class="login-modal">
		...
		<div class="login_leftPage login_forget">
			...
			<div class="login-body">
				...
				<button class="login_leftPageGetcode" id="getmobyzm" onclick="verification('/<?php echo (API_PATH); ?>/微信Sms/sendMessage')">获取验证码</button>
...
<script type="text/javascript">
	...
	function getUser() {
		...
		if(ifWeChatLogin == 1) {
			...
			$.get(url, function (res) {
				window.location.href="/index.php/Pc/Activity/data_list/channel/hd"
			})
		}
	}
	function  WeChatLogin() {
		...
		$.get(url, function (res) {
			if(res.result == 1) {
				window.location.href="https://open.weixin.qq.com/connect/qrconnect?appid="+res.data['appid']+"&redirect_uri=http://"+res.data['area_name']+"//index.php/Pc/Activity/data_list/channel/hd/ifWeChatLogin/1&response_type=code&scope=snsapi_login&state=2015#wechat_redirect";
			} else {
				layer.msg("系统繁忙请稍后再试！");
			}
		})
...
<script>
	var vm = new Vue({
		...
		methods:{
			...
			search:function (id){
				window.location.href="/index.php/Pc/Activity/detail/channel/<?php echo ($channel); ?>/type/1/data_id/"+id;
			}
		}
	});
</script>