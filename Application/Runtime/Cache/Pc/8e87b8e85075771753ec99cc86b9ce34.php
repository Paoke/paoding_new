...
<section id="template">
	<div class="science_list outside">
		<div class="center">
			<div class="blog_box">
				<a href="/index.php/Pc/Article/data_list/channel/hzqy">合作企业 > </a>
				<span>企业详细</span>
			</div>
			<div class="blog">
			...
			<div class="blog_widget">
				...
				<!--<div>技术领域：</div>-->
					<!--<template v-if="listData.status==0">-->
						<!--<div>成熟度：原型方案</div>-->
					<!--</template>-->
					<!--<template v-if="listData.status==1">-->
						<!--<div>成熟度：成熟方案</div>-->
					<!--</template>-->
					<!--<template v-if="listData.status==-1">-->
						<!--<div>成熟度：产品成型</div>-->
				...
				<!--<div>交付形式：{{listData.jfxs}}</div>-->
					<!--<template v-if="listData.zhuanli==1">-->
					<!--<div>专利：有</div>-->
					<!--</template>-->
					<!--<template v-if="listData.zhuanli==0">-->
						<!--<div>专利：无</div>-->
					<!--</template>-->
				<!--</div>-->
				<div class="widget_footer">
					<p>需要了解此技术或更多需求，请联系我们</p>
					<button class="widget_sub">联系我们</button>
				</div>
			</div>
		</div>
	</div>
...
<script>
	...
	function getUser() {
		...
		if(ifWeChatLogin == 1) {
			...
			$.get(url, function (res) {
				window.location.href="/index.php/Pc/Article/detail/channel/hzjg/type/1/data_id/<?php echo ($data_id); ?>"
			})
		}
	}
	function  WeChatLogin() {
		...
		$.get(url, function (res) {
			if(res.result == 1) {
				window.location.href="https://open.weixin.qq.com/connect/qrconnect?appid="+res.data['appid']+"&redirect_uri=http://"+res.data['area_name']+"//index.php/Pc/Article/detail/channel/hzjg/type/1/data_id/<?php echo ($data_id); ?>/ifWeChatLogin/1&response_type=code&scope=snsapi_login&state=2015#wechat_redirect";
			} else {
				layer.msg("系统繁忙请稍后再试！");
			}
		})
	...
	var vm = new Vue({
		...
		methods:{
			getData:function () {
				var _this = this;
				var url = "/<?php echo (API_PATH); ?>/ChannelIndex/index/action/dataDetail/channel/hzjg/type/1/data_id/<?php echo ($data_id); ?>";
				$.get(url, function (res) {
			...
			getConfigData:function () {
				var _this = this;
				var url = "/<?php echo (API_PATH); ?>/Setting/settingAbout/channel/hzjg/type/1";
				$.get(url, function (res) {
