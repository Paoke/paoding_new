...
<script type="text/javascript">
    $(function(){
        var url = "/index.php/Admin/Activity/tags/action/extends/channel/<?php echo ($channel); ?>/type/<?php echo ($type); ?>/id/<?php echo ($id); ?>";
        var id = $("#id").val();
    ...
    function ajax_submit_form(form_id) {
        ...
        if (id == '') {
            //不存在，表示添加
          action = "/index.php/Admin/Activity/tags/action/add/channel/<?php echo ($channel); ?>/type/<?php echo ($type); ?>";
        } else {
            //存在，表示编辑
           action = "/index.php/Admin/Activity/tags/action/edit/channel/<?php echo ($channel); ?>/type/<?php echo ($type); ?>/id/<?php echo ($id); ?>";
        }
        //异步提交表单数据
        $.ajax({
            type: "post",
            ...
            success: function (res) {
                if (res.result == 1) {
                    layer.msg(res.msg);
                    setTimeout(function () {
                        window.location.href = "/index.php/Admin/Activity/tags/action/page_list/channel/<?php echo ($channel); ?>/type/<?php echo ($type); ?>";
                    }, 1000);
                }
                if (res.result == 0) {
                    layer.msg(res.msg);
