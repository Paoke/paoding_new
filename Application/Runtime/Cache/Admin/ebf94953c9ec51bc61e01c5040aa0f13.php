                    ...
                    <section class="panel">
                        <header class="panel-heading panel-body">
                                    ...
                                    <div class="pull-right">
                                        <div class="btn-group">
                                            <a href="/index.php/Admin/Admin/group/action/page_add" class="btn btn-default "><i
                                                    class="fa fa-plus"></i>新增工作组</a>
                                        </div>
                                    </div>
                        </header>
                        ...
                        <div class="">
                            <div class="adv-table">
                                <table id="list-table" class="table table-bordered table-hover" role="grid"
                                    ...
                                    <thead>
                                    <tr role="row">
                                        <th width="8%" valign="middle">ID</th>
                                        <th width="20%">组织机构简称</th>
                                        <th width="22%">组织机构全称</th>
                                        <th width="10%" valign="middle">创建人</th>
                                        <th width="15%" valign="middle">创建时间</th>
                                        <th width="10%" valign="middle">排序</th>
                                        <th width="15%" valign="middle">操作</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php if(is_array($info)): foreach($info as $k=>$vo): ?><tr role="row" align="center" class="<?php echo ($vo["level"]); ?>" id="<?php echo ($vo["id"]); ?>">
                                            <td><?php echo ($vo["id"]); ?></td>
                                            <td align="left" onclick="listSon(this,'<?php echo ($vo["id"]); ?>')">
                                                <?php if($vo["infoSon"] >= 1): ?><span class="fa fa-caret-right" id="icon_<?php echo ($vo["level"]); ?>_<?php echo ($vo["id"]); ?>"
                                                          onclick="listSon(this,'<?php echo ($vo["id"]); ?>')">&nbsp;</span><?php endif; ?>
                                                <?php echo (getSubstr($vo["name"],0,20)); ?>
                                            </td>
                                            ...
                                            <td>
                                                <input type="text"
                                                       onchange="updateSort('','manage_group','id','<?php echo ($vo["id"]); ?>','sort',this)"
                                                       onkeyup="this.value=this.value.replace(/[^\d]/g,'')" maxlength="4" size="4"
                                                       value="<?php echo ($vo["sort"]); ?>"
                                                       class="input-sm"/>
                                            </td>
                                            <td>
                                                <div class="btn-group">
                                                    <a class="btn btn-default"
                                                       href="/index.php/Admin/Admin/group/action/page_edit/id/<?php echo ($vo["id"]); ?>" title="编辑"><i
                                                            class="fa fa-pencil"></i></a>
                                                    <a href="#delModal" data-toggle="modal" class="btn btn-default"
                                                       data-name="<?php echo ($vo["name"]); ?>" title="删除"
                                                       data-url="/index.php/Admin/Admin/group/action/del/id/<?php echo ($vo["id"]); ?>"
                                    ...
                                    <div class="span6">
                                        <div class="dataTables_paginate paging_bootstrap pagination">
                                            <ul>
                                                ...
                                                <?php if($page < 5): ?><!-- 页码条 -->
                                                    <?php $__FOR_START_21515__=1;$__FOR_END_21515__=$page+1;for($i=$__FOR_START_21515__;$i < $__FOR_END_21515__;$i+=1){ ?><!-- 循环四条以内 -->
                                                        <?php if($i == $page_now ): ?><li class="active"><a href="#"><?php echo ($i); ?></a></li>
                                                            <?php elseif($i < $page_now ): ?>
                                                            <li><a href="<?php echo U('Admin/User/index/page_now');?>/<?php echo ($i); ?>/page_num/<?php echo ($page_num); ?>"><?php echo ($i); ?></a>
                                                            </li>
                                                            <?php else: ?>
                                                            <li><a href="<?php echo U('Admin/User/index/page_now');?>/<?php echo ($i); ?>/page_num/<?php echo ($page_num); ?>"><?php echo ($i); ?></a>
                                                            </li><?php endif; } ?>
                                                    <?php elseif($page_now < 3): ?>
                                                    <?php $__FOR_START_21224__=1;$__FOR_END_21224__=6;for($i=$__FOR_START_21224__;$i < $__FOR_END_21224__;$i+=1){ ?><!-- 循环1-5 -->
                                                    ...
                                                    <?php $__FOR_START_11521__=$page_now-3;$__FOR_END_11521__=$page+1;for($i=$__FOR_START_11521__;$i < $__FOR_END_11521__;$i+=1){ ?><!-- 循环当前页为倒数第二页时 -->
                                                        <?php if($i == $page_now ): ?><li class="active"><a href="#"><?php echo ($page_now); ?></a></li>
                                                            <?php elseif($i < $page ): ?>
                                                            <li><a href="<?php echo U('Admin/User/index/page_now');?>/<?php echo ($i); ?>/page_num/<?php echo ($page_num); ?>"><?php echo ($i); ?></a>
                                                            </li>
                                                    ...
                                                    <?php $__FOR_START_10150__=$page_now-4;$__FOR_END_10150__=$page+1;for($i=$__FOR_START_10150__;$i < $__FOR_END_10150__;$i+=1){ ?><!-- 循环当前页为最后一页时 -->
                                                        <?php if($i == $page_now ): ?><li class="active"><a href="#"><?php echo ($page_now); ?></a></li>
                                                            <?php elseif($i < $page ): ?>
                                                            <li><a href="<?php echo U('Admin/User/index/page_now');?>/<?php echo ($i); ?>/page_num/<?php echo ($page_num); ?>"><?php echo ($i); ?></a>
                                                            </li>
                                                    ...
                                                    <?php else: ?>
                                                    <?php $__FOR_START_9703__=$page_now-2;$__FOR_END_9703__=$page_now+3;for($i=$__FOR_START_9703__;$i < $__FOR_END_9703__;$i+=1){ ?><!-- 循环除了前五条 和后五条 -->
...
<script type="text/javascript">
    ...
    function listSon(obj, id) {
        var str = '';
        var url = "/index.php/Admin/Admin/group/action/page_list/id/" + id;
        ...
        if (faClass == "fa fa-caret-right") {
            $.get(url, function (res) {
                $.each(res.data, function (i, item) {
                    ...
                    if (item.infoSon >= 1) {
                        ...
                        str += item.name.substr(0, 20) + '</td>';
                    } else {
                        str += '<td align="left" style="padding-left:' + item.level * 15 + 'px;">';
                        str += item.name.substr(0, 20) + '</td>';
                    }
                    str += '<td>' + item.full_name + '</td>';
                    str += '<td>' + item.create_user + '</td>';
                    str += '<td>' + item.create_time + '</td>';
                    str += '<td><input type="text" onchange=\'updateSort("","system_module","id","' + item.id +
                            '","sort",this)\' onkeyup="this.value=this.value.replace(/[^\d]/g,"")" maxlength="4" ' +
                            'size="4" value="' + item.sort + '" class="input-sm"/></td>';
                    str += '<td><div class="btn-group btn-group-sm">';
                    str += '<a class="btn btn-default" ' +
                            'href="/index.php/Admin/Admin/group/action/page_edit/id/' + item.id + '"' +
                    ...
                    str += '<a href="#delModal" data-toggle="modal" class="btn btn-default delModal"' +
                            ...
                            'data-url="/index.php/Admin/Admin/group/action/del/id/' + item.id + '"' +
            ...
            var url = "/index.php/Admin/Admin/group/action/page_list/id/" + id + "/getTreeSon/1";
