<?php if (!defined('THINK_PATH')) exit();?><!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>技术项目</title>
    <meta name="description" content="庖丁科技众包服务平台携手国内外先进技术科研院所、顶级技术专家，以实现科技成果市场化为核心，为企业提供快速精准的需求匹配服务，从而实现企业以及科技资源的有效对接，帮助企业实现产业技术升级，助力先进技术完成产业化发展。" />
    <meta name="keywords" content="庖丁众包、智能技术、机械制造、健康医疗、材料科学、能源环保、生产流程优化"/>
    <meta name='viewport' content='user-scalable=no,width=750'>
    <link rel="stylesheet" href="<?php echo (MOBILE); ?>/css/common_new.css">
    <link rel="stylesheet" href="<?php echo (MOBILE); ?>/css/index.css">
</head>
<body>
<div class="mask" style="display:none"></div>
<div class="contackWay_alert">
    <p class="contackWay">联系方式</p>
    <div class="alert_part clear">
        <p class="alert_part_l fl">联系人</p>
        <p class="alert_part_r fl">张东伟</p>
    </div>
    <div class="alert_part clear">
        <p class="alert_part_l fl">手机号</p>
        <p class="alert_part_r fl">1852020174</p>
    </div>
    <div class="alert_part clear">
        <p class="alert_part_l fl">邮箱号</p>
        <p class="alert_part_r fl">21343@qq.com</p>
    </div>
    <div class="alert_part clear">
        <p class="alert_part_l fl">企业名</p>
        <p class="alert_part_r fl">广州中国科技园先进技术研究所</p>
    </div>
    <div class="close">关闭</div>
</div>
<div class="banner">
    <img src="<?php echo (MOBILE); ?>/images/index-techdetail-image1.jpg" alt="">
</div>

<div class="share clear">
    <div class="share_left fl">
        <p class="title">阻燃零甲醛环保发泡板材</p>
        <p class="tip">
            发布于 <span>2017-09-26</span>
            浏览  <span>2038</span>
        </p>
    </div>
    <div class="fr">
        <img src="<?php echo (MOBILE); ?>/images/icon-index-techdetail-share.jpg" alt="">
        <p class="tip">分享</p>
    </div>
</div>

<div class="index_project_data clear">
    <div class="data_section fl first_data">
        <p class="sec_num">合作形式</p>
        <p class="sec_status">待商议</p>
        <p class="sec_line"></p>
    </div>
    <div class="data_section fl">
        <p class="sec_num">交付形式</p>
        <p class="sec_status">待商议</p>
        <p class="sec_line"></p>
    </div>
    <div class="data_section fl">
        <p class="sec_num">合作价格</p>
        <p class="sec_status">待商议</p>
    </div>
</div>
<div class="index_div"></div>

<div class="intro">
    <div class="intro_section clear">
        <p class="intro_left fl">技术领域</p>
        <p class="intro_right fr">应用类</p>
    </div>

    <div class="intro_section clear">
        <p class="intro_left fl">成熟度</p>
        <p class="intro_right fr">批量销售</p>
    </div>

    <div class="intro_section clear">
        <p class="intro_left fl">应用行业</p>
        <p class="intro_right fr">建筑、环境装饰</p>
    </div>

    <div class="intro_section clear">
        <p class="intro_left fl">专利证书</p>
        <p class="intro_right fr">23项</p>
    </div>

    <div class="intro_section clear">
        <p class="intro_left fl">联系方式</p>
        <p class="intro_right fr" id="getContackWay" style="color:#ff971d">获取联系方式</p>
    </div>
</div>
<div class="index_div"></div>

<div class="tec_tab">
    <a href="#tip1" class="active"><p class="tab_p">技术讲述</p></a>
    <a href="#tip2"><p class="tab_p">技术优势</p></a>
    <a href="#tip3"><p class="tab_p">效果指标</p></a>
    <a href="#tip4"><p class="tab_p">应用案例</p></a>
    <div class="slideBlock"></div>
</div>

<a name="tip1">
    <div class="tec_main">
        <div class="tec_tit">技术详述</div>
        <div class="tec_content">
            <p class="tec_ct_tit">一、技术简介</p>
            <p class="tec_ct_p">本项目开发一种阻燃零甲醛环保发泡板材系列产品，以生态木质生物质为绿色原料，通过近8年来自主创新的蒸汽动态发泡技术制造，体现科技创新、低碳环保、性能优势和成本价格等优势，可实现超低、中和高密度三种零甲醛阻燃发泡板材系列产品制造，可分别替代传统发泡塑料板、无机纤维保温板、传统人造板在家具、建筑节能环保及高级装饰等领域广泛应用。新产品的核心技术属国内外独创，处于国际领</p>
            <p class="tec_ct_tit">二、使用方法</p>
            <p class="tec_ct_p">
                （1）将本涂料（A/B/C/D）按照规定比例混合后，通过高速搅拌混合均匀，然后加入专用溶剂再次搅拌，在规定温度下熟化后即可使用；<br>
                （2）基材表面处理：需要除去基材表面的油污等，方法喷砂、砂纸打磨等；<br>
                （3）根据基材形状，可用采用喷涂、静电喷涂、电泳等工艺；<br>
                （4）喷涂到基材上之后，先在常温下干燥，之后放入80度的烘箱烘2~3分钟，在经过140~170度高温固化30分钟左右即可。
            </p>
            <p class="tec_ct_tit">三、技术团队</p>
            <p class="tec_ct_p">
                技术团队成员以现有的5名具有研究生学历的技术研发人员为主，3名工程师和1名管理人员，以10名在本领域具有丰富工程经验的教授和具有10年以上企业生产和工程技术高级人才为规模组成项目研发的顾问团队，共同构建本项目的科技创新与工程团队。
            </p>
        </div>
    </div>
</a>

<a name="tip2">
    <div class="tec_main">
        <div class="tec_tit">技术优势</div>
        <div class="tec_content">
            <p class="tec_ct_tit">一、本技术产品具有以下创新点</p>
            <p class="tec_ct_p">
                １.技术创新<br>
                零甲醛、阻燃、绿色环保
                保温、降噪优异
                多元化密度80-850kg/m3 <br>
                2. 工艺创新<br>
                循环利用、清洁生产
                可实现 “卷-卷”大规模生产<br>
                3. 应用创新<br>
                可替代传统玻璃棉板、泡沫塑料板、人造板等在家具、建筑、公共场所大规模应用。
            </p>
        </div>
    </div>
</a>

<a name="tip3">
    <div class="tec_main">
        <div class="tec_tit">效果指标</div>
        <div class="tec_content">
            <p class="tec_ct_tit">一、本产品已进入量产阶段</p>
            <img src="<?php echo (MOBILE); ?>/images/index-techdetail-image2.jpg" alt="">
            <img src="<?php echo (MOBILE); ?>/images/index-techdetail-image3.jpg" alt="">
        </div>
    </div>
</a>

<a name="tip4">
    <div class="tec_main">
        <div class="tec_tit">应用案例</div>
        <div class="tec_content">
            <p class="tec_ct_tit">一、应用案例1</p>
            <img src="<?php echo (MOBILE); ?>/images/index-techdetail-image4.jpg" alt="">
            <img src="<?php echo (MOBILE); ?>/images/index-techdetail-image5.jpg" alt="">
        </div>
    </div>
</a>

<p class="advert">
    庖丁众包·专业科技服务平台
</p>

<div class="sec_return">
    <a onclick="javascript:history.go(-1);">
        <img class="return_icon" src="<?php echo (MOBILE); ?>/images/icon-common-return.png" alt="">
    </a>
    <p class="collect">收藏</p>
</div>
</body>
<script src='<?php echo (MOBILE); ?>/js/jquery-3.0.0.min.js'></script>
<script>

    // 导航栏
    // 需修改的元素：dv、 body的padding-top值，赋为dv的高度
    $(function () {
        var ie6 = /msie 6/i.test(navigator.userAgent),
            dv = $('.tec_tab'), //活动元素
            st;
        dv.attr('otop', dv.offset().top); //存储原来的距离顶部的距离
        $(window).scroll(function () {
            // 监听滚动  nav字体颜色   四个锚点分别监听位置判断
            // console.log($(".tec_tit").offset().top - document.body.scrollTop);

            st = Math.max(document.body.scrollTop || document.documentElement.scrollTop);
            if (st >= parseInt(dv.attr('otop'))) {
                if (ie6) {
                    //IE6不支持fixed属性，所以只能靠设置position为absolute和top实现此效果
                    dv.css({ position: 'absolute', top: st });
                }
                else if(dv.css('position') != 'fixed'){
                    dv.css({ 'position': 'fixed', 'top': 0,'left':'50%','margin-left':'-375px'});
                    $('body').css('padding-top','90px');
                }
            } else if (dv.css('position') != 'relative'){
                dv.css({ 'position': 'relative' });
                $('body').css('padding-top','');
            }
        });
    });


    $(".tec_tab a").on("click",function(){
        var index = $(this).index();
        var tranX = index*188;
        $(".slideBlock").css("transform","translateX("+tranX+"px)");
        $(this).addClass('active').siblings().removeClass('active');
    });


    $(".close").on("click",function(){
        $(".mask").hide();
        $(".contackWay_alert").hide();
    });
    $(".mask").on("click",function(){
        $(".mask").hide();
        $(".contackWay_alert").hide();
    });
    $("#getContackWay").on("click",function(){
        $(".mask").show();
        $(".contackWay_alert").show();
    });

</script>
</html>