...
<section id="template">
	<body>
		<div class="web">
			...
			<div class="index_head">
				<div class="swiper-container">
					<div class="swiper-wrapper">
				        ...
				        <a href="/index.php/Mobile/Article/data_list/channel/xq" data-i="0" class="swiper-slide index_head_active">最新</a>
						</li>
				      <li v-for="category in categoryData" class="science_tabLi">
				        	<a @click="getDataByCategory(category.id)"  v-bind:id="category.id" class="swiper-slide">{{category.cat_name}}</a>
					  </li>
				  </div>
				</div>
			...
			<div class="index_body">
				<div data-id="0" class="weui-panel weui-panel_access index_section index_section_active" data-page="1" type="new">
		            <div class="weui-panel__bd" id="Dissimilarity">
		            	<li v-for="list in listData">
			                <a @click="getDataDetail(list.id)" class="weui-media-box weui-media-box_appmsg">
			                ...
			                <div class="index_sectionB">
								...
								<!--<p class="index_sectionB3"><?php echo ($v["coop_type"]); ?></p>-->
							</div>
						</li>
		            </div>
		        ...
		        <?php if(is_array($others['data'])): foreach($others['data'] as $k=>$val): ?><div data-id="<?php echo ($k); ?>" class="weui-panel weui-panel_access index_section" data-page="1" type="<?php echo ($k); ?>">
			            <div class="weui-panel__bd">
			            	<?php if(is_array($val)): $i = 0; $__LIST__ = $val;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Task/task_info',array('category_id'=>$v['indus_pid'],'task_id'=>$v['task_id']));?>" class="weui-media-box weui-media-box_appmsg">
				                    <div class="weui-media-box__hd" style="width: 100px;height: 75px;">
				                        <img style="height: 100%;" class="weui-media-box__thumb" src="<?php echo ($v["task_pic"]); ?>">
				                    </div>
				                    <div class="weui-media-box__bd">
				                        <h4 class="weui-media-box__title"><?php echo ($v["task_title"]); ?></h4>
				                        <p class="weui-media-box__desc"><?php echo ($v["task_desc"]); ?></p>
				                    </div>
				                </a>
				                <div class="index_sectionB">
									<p class="index_sectionB1"><?php echo ($v["indus_name"]); ?></p>
			...
			<a href="/index.php/Mobile/Index/science_xq"><div class="index_send"><p>发布需求</p></div></a>
			<div class="index_foot">
				<div class="weui-tabbar">
	                ...
	                <a href="/index.php/Mobile/Article/data_list/channel/xq" class="weui-tabbar__item">
	                    <img src="/Template/5u/mobile/Static/img/index_need-active.png" alt="" class="weui-tabbar__icon">
	...
	var vm = new Vue({
		...
		methods:{
			...
			getDataByCategory:function(category_id){
				...
				$('#' + category_id).addClass('index_head_active');
			},
...
<script type="text/javascript">
	function submit(){
		...
		var html='<li class="science_tabLi"><a href="/index.php/Mobile/Article/xq_list" data-i="0" class="swiper-slide index_head_active">最新</a></li>';
		for(var i=0; i<obj.length; i++){
			if(obj[i].checked){
				html+='<li class="science_tabLi"><a onclick="getDataByCategory('+obj[i].id+')" id="'+obj[i].id+'" class="swiper-slide">'+obj[i].value+'</a></li>';
			}
		}
		$(".swiper-wrapper").html(html);
	}
	function getDataByCategory(id){
		var url = "/<?php echo (API_PATH); ?>/ChannelIndex/index/action/dataList/channel/xq/type/1/category_id/"+id;
		var data={
			"order_field":"create_time",
			"order_by":"DESC",
			"page": 1,
