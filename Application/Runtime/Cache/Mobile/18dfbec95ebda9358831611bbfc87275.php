...
<head>
	...
	<body>
		<div class="web">
			<?php if($task != null): ?><div class="weui-panel weui-panel_access" style="background: #f3f4f8;margin-top: 0px;" data-page="1">
					<?php if(is_array($task)): $i = 0; $__LIST__ = $task;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><div class="weui-panel__hd user_head"><?php echo ($v["task_title"]); ?></div>
	        ...
	        <?php else: ?>
				<div class="uesr_null">
					...
					<div class="uesr_null1">您暂未提交任何方案</div>
					<div class="uesr_null2"><a style="color:blue;text-decoration:none;" href="/index.php/Mobile/Index/science">返回技术列表</a></div>
				</div><?php endif; ?>
		</div>
	</body>
...
<script type="text/javascript">
		function pullUpAction (dom,_s) {
			...
			$.ajax({
				url:"<?php echo U('User/user_tasks');?>",
				data:{page:pageNum++},
				type:'post',
				dataType:'json',
		...
		function addHtml(datas,dom,_s){
	        ...
	        $(_s).parent().siblings(".weui-panel_access").append(result);
	        $(_s).siblings(".index_body_foot2").css("display","none");
