 ...
 * @callback string  回调函数(单张图片返回保存路径字符串，多张则为路径数组 )
 */
function GetUploadifyImage(num, elementid, path, callback) {
    var upurl = '/index.php?m=Admin&c=Uploadify&a=uploadImg&num=' + num + '&eid=' + elementid + '&path=' + path + '&func=' + callback;
    var iframe_str = '<iframe frameborder="0" ';
    iframe_str = iframe_str + 'id=uploadify ';
    iframe_str = iframe_str + ' src=' + upurl;
    iframe_str = iframe_str + ' allowtransparency="true" class="uploadframe" scrolling="no"> ';
 ...
 * @param config UEditor配置项
 * **/
function initUEditor(site, element_id, config){
    if(isEmpty(site)){
       site = '5u';
    }
    ...
    config['serverUrl'] = "/Public/js/ueditor/php/controller.php?site="+site
    var editor = UE.getEditor(element_id, config);
...
function layer_window(title, url){
    //多窗口模式，层叠置顶
