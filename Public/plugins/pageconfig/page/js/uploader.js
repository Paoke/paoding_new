$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: null, //发送地址
        filetypes: "gif,jpg,jpeg,png,bmp,flv,JPG,JPEG,PNG,GIF,BMP,mp3,mp4,avi", //文件类型
        filesize: "10240", //文件大小
        btntext: "浏览...", //上传按钮的文字
    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            var btnObj;
            if (p.multiple) {
                var btnObj = $('<button class="">' + p.btntext + '</button>').appendTo(parentObj);
            } else {
                btnObj = parentObj;
            }
            //初始化属性
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    //显示单张图片
                    if (p.action == 1) {
                        setSingleImage(data.path, data.thumb);
                    }
                    //显示音乐路径
                    if (p.action == 2) {
...
function addImage(targetObj, originalSrc, thumbSrc) {
    //插入到相册UL里面
    var newLi = $('<li>'
    + '<input type="hidden" id="hid_photo_name" name="hid_photo_name" value="0|' + originalSrc + '|' + thumbSrc + '" />'
...
//设置头像
function setSingleImage(originalSrc, thumbSrc) {
    $("#singleImage").attr("src", thumbSrc);
    $("#txtSingleImage").attr("value", thumbSrc);
}

//设置头像
function setFilePath(path) {
    $("#txtMusic").attr("value", path);
}